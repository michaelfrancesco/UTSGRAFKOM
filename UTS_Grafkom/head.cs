...
namespace Tugas_Individu
{
    class head : Mesh
    {
        ...
        public head(List<Vector3> vertices, List<Vector3> textureVertices, List<Vector3> normals, List<uint> vertexIndices, int vertexBufferObject, int vertexArrayObject, Shader shader, int elementBufferObject, Matrix4 transform)
        {
        ...
        public head(string color = "red", string name = "allen")
        {
            this.color = color;
            this.name = name;
        }
        ...
        public void createHalfEllipsoidVetices(float _positionX = 0.0f, float _positionY = 0.4f, float _positionZ = 0.0f, float _radius = 0.4f)
        {
            ...
            for (float v = -_pi / 2; v <= -_pi / 8; v += 0.0001f)
            {
